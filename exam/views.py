...
def main(request):
    ...
    for y in year:
        exam = Exam.objects.filter(year__year = y['year'], start_date_time__date=timezone.now(),
                                   start_date_time__lte=datetime.datetime.now(), end_date_time__gte=datetime.datetime.now())
...
def add_questions(request):
    ...
    exam = Exam.objects.all() #filter(student.year=exam.year)
    return render(request, 'exam/questions.html', {'exam':exam})
